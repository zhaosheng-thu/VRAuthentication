...
def main():
    ...
    augmentation_time = 10  # guassion_aug
    ...
    for threshold in thresholds:  # 设置合理的阈值 :
        ...
        for pin in all_pin_list:
            ...
            for segment in range(n_segments):
                joblib.dump(classifiers[segment], f"weights/classifier_segment_{segment}.pkl")
                joblib.dump(scalers[segment], f"weights/scaler_segment_{segment}.pkl")

            print("Classifiers and scalers have been saved.")
            ## with probability
            # 对测试集进行概率投票
            final_predictions = []
