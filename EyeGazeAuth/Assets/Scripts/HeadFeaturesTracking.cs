...
public class HeadFeaturesTracking : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (Begin)
        {
            ...
            InterestedFeature h = new InterestedFeature(pos, rot);
            headFeatures.Add(h);
    ...
    {
        pathHead = @"E:\Desktop\data\VRAuth1\P" + trackingScript.Name.Split("-")[1] + @"\Head_data_" + Name + ".csv";
        pathNeck = @"E:\Desktop\data\VRAuth1\P" + trackingScript.Name.Split("-")[1] + @"\Neck_data_" + Name + ".csv";
        ...
        pathHead = @"E:\Desktop\data\VRAuth1angle\P" + trackingScript.Name.Split("-")[1] + @"\Head_data_" + Name + ".csv";
        using (StreamWriter writer = new StreamWriter(pathHead))
        {
            // 写入CSV文件的标题行
            writer.WriteLine("Yaw,Pitch,Roll");
            // 写入数据行
            foreach (var data in headFeatures)
            {
                Vector3 rot = data.QuaternionData.eulerAngles;
                writer.WriteLine($"{rot.y},{rot.x},{rot.z}");
            }
