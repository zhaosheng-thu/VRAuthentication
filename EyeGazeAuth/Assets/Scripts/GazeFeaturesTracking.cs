...
public class GazeFeaturesTracking : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        leftEye = GameObject.Find("LeftEyeController");
        rightEye = GameObject.Find("RightEyeController");
    ...
    {
        pathStr = @"E:\Desktop\data\VRAuth1\P" + trackingScript.Name.Split("-")[1] + @"\GazeCalculate_data_" + Name + ".csv";
        ...
        pathStr = @"E:\Desktop\data\VRAuth1\P" + trackingScript.Name.Split("-")[1] + @"\GazeRaw_data_" + Name + ".csv";
        using (StreamWriter writer = new StreamWriter(pathStr))
        {
            // 写入CSV文件的标题行
            writer.WriteLine("L-QuaternionX,L-QuaternionY,L-QuaternionZ,L-QuaternionW,R-QuaternionX,R-QuaternionY,R-QuaternionZ,R-QuaternionW,");
        ...
        pathStr = @"E:\Desktop\data\VRAuth1angle\P" + trackingScript.Name.Split("-")[1] + @"\GazeCalculate_data_" + Name + ".csv";
        using (StreamWriter writer = new StreamWriter(pathStr))
        {
            // 写入CSV文件的标题行
            writer.WriteLine("L_Yaw,L_Pitch,L_Roll,R_Yaw,R_Pitch,R_Roll,");
            // 写入数据行
            foreach (var data in gazeFeaturesCalculate)
            {
                Vector3 rotL = data.LeftQuat.eulerAngles;
                Vector3 rotR = data.RightQuat.eulerAngles;
                writer.WriteLine($"{rotL.y},{rotL.x},{rotL.z},{rotR.y},{rotR.x},{rotR.z}");
            }
            Debug.Log("Finish Writing GazeFeaturesCal");
        }

        pathStr = @"E:\Desktop\data\VRAuth1angle\P" + trackingScript.Name.Split("-")[1] + @"\GazeRaw_data_" + Name + ".csv";
        using (StreamWriter writer = new StreamWriter(pathStr))
        {
            writer.WriteLine("L_Yaw,L_Pitch,L_Roll,R_Yaw,R_Pitch,R_Roll,");
            // 写入数据行
